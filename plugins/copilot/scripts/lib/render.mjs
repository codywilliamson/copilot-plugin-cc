...
function formatCopilotResumeCommand(job) {
  ...
  return `copilot --resume ${job.sessionId}`;
}
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.sessionId) {
    lines.push(`  Copilot session ID: ${job.sessionId}`);
  }
...
export function renderSetupReport(report) {
  ...
  if (report.defaultEffort) {
    lines.push(`- default effort: ${report.defaultEffort}`);
  }
  lines.push("");
